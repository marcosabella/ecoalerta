...
// The public OSRM map-matching service can be configured with a lower trace
// limit than OSRM's default. Keep enough headroom for the shared demo server.
const MAX_MATCH_POINTS = 45;
...
export function RouteDesignerMap({ route, stops, municipality, onSave, onAddStop, notify, onCancel }) {
  ...
  const snapToRoads = async () => {
    ...
    try {
      ...
      const radiuses = trace.map(() => 25).join(';');
      // Keep only the snapped samples. The route geometry generated by OSRM
      // can add connecting streets that were never part of the drawn trace.
      const response = await fetch(`https://router.project-osrm.org/match/v1/driving/${coordinates}?overview=false&geometries=geojson&gaps=split&tidy=false&radiuses=${radiuses}`, { signal: controller.signal });
      const result = await response.json();
      if (!response.ok || result.code !== 'Ok') throw new Error(result.message || 'No se pudo ajustar el recorrido a las calles.');
      const snappedTrace = trace.map((point, index) => {
        const location = result.tracepoints?.[index]?.location;
        return location ? [location[1], location[0]] : point;
      });
      const matchedPoints = result.tracepoints?.filter(Boolean).length || 0;
      if (matchedPoints < 2) throw new Error('No se encontraron suficientes calles cerca de la traza.');
      const distance = snappedTrace.slice(1).reduce((sum, point, index) => sum + distanceBetween(snappedTrace[index], point), 0);
      setPath(snappedTrace); setRouteMeta({ distance: Math.round(distance), duration: null });
      notify('Traza emprolijada sobre las calles que marcaste.');
    } catch (error) {
      notify(error.name === 'AbortError' ? 'El servicio de calles tardó demasiado. Volvé a intentar.' : `No se pudo ajustar a calles: ${error.message}`);
    } finally {
